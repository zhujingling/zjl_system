...
import com.zjlsystem.entity.shiro.SystemUserSession;
import com.zjlsystem.shiro.CustomShiroSessionDAO;
...
public class CustomSessionManager {
	 ...
	 * @return
	 */
	public List<SystemUserSession> getAllUser() {
		// 获取所有session
		Collection<Session> sessions = customShiroSessionDAO.getActiveSessions();
		List<SystemUserSession> list = new ArrayList<SystemUserSession>();

		for (Session session : sessions) {
			SystemUserSession item = getSessionBo(session);
			if (null != item) {
				list.add(item);
			}
		}
	...
	public List<SimplePrincipalCollection> getSimplePrincipalCollectionByUserId(Long... userIds) {
		...
		for (Session item : sessions) {
			...
			Object obj = item.getAttribute(DefaultSubjectContext.PRINCIPALS_SESSION_KEY);
	 ...
	 * @return
	 */
	public SystemUserSession getSession(String sessionId) {
		Session session = shiroSessionRepository.getSession(sessionId);
		SystemUserSession systemUserSesion = getSessionBo(session);
		return systemUserSesion;
	}

	private SystemUserSession getSessionBo(Session session) {
		// 获取session登录信息。
		...
		if (obj instanceof SimplePrincipalCollection) {
			...
			if (null != obj && obj instanceof SystemUser) {
				// 存储session + user 综合信息
				SystemUserSession item = new SystemUserSession((SystemUser) obj);
				// 最后一次和系统交互的时间

				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				item.setLastAccess(sdf.format(session.getLastAccessTime()));
				// 主机的ip地址
				item.setHost(session.getHost());
				// session ID

				item.setSessionId(session.getId().toString());
				// session最后一次与系统交互的时间
				item.setUserLastLoginTime(sdf.format(session.getLastAccessTime()));
				// 回话到期 ttl(ms)
				item.setTimeout(session.getTimeout());
				// session创建时间
				item.setStartTime(sdf.format(session.getStartTimestamp()));
				// 是否踢出
	...
	public void forbidUserById(String id, Long status) {
		// 获取所有在线用户
		for (SystemUserSession item : getAllUser()) {
			String userId = item.getId();
			// 匹配用户ID
			if (userId.equals(id)) {
				// 获取用户Session
				Session session = shiroSessionRepository.getSession(item.getSessionId());
				// 标记用户Session
				SessionStatus sessionStatus = (SessionStatus) session.getAttribute(SESSION_STATUS);
