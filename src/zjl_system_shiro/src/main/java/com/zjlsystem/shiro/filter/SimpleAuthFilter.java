...
public class SimpleAuthFilter extends AccessControlFilter {
	...
	protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
		 ...
		 * 然后：{@link UserLoginController.submitLogin(...)}中的 <code>String url =
		 * WebUtils.getSavedRequest(request).getRequestUrl();</code>
		 * 如果还有问题，请咨询我。
		 */
		WebUtils.getSavedRequest(request);
		// 再重定向
		WebUtils.issueRedirect(request, response, "/system/kickedOut.shtml");
		return false;
	}
