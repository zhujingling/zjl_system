...
import com.zjlsystem.tool.common.SpringContextUtils;
...
public class TokenManager {
	// 用户登录管理
	public static final SampleRealm realm = SpringContextUtils.getBean("sampleRealm", SampleRealm.class);
	// 用户session管理
	...
	public static String getUserRealName() {
		return getToken().getUserRealName();
	}
	...
	public static SystemUser login(SystemUser user, Boolean rememberMe) {
		ShiroToken token = new ShiroToken(user.getUserAccount(), user.getUserPassword());
	...
	/*
	 * 清空当前用户权限信息。 目的：为了在判断权限的时候，再次会再次 <code>doGetAuthorizationInfo(...)
	 * </code>方法。 ps： 当然你可以手动调用 <code> doGetAuthorizationInfo(...) </code>方法。
	...
	public static void clearNowUserAuth() {
		 ...
		 * 或者它的实现子类的{Collection<Realm> getRealms()}方法获取）。 获取到的时候是一个集合。Collection
		 * <Realm> RealmSecurityManager securityManager = (RealmSecurityManager)
